...
---
# 方块生成
方块在10列的第3-6列生成，这些列的第20行受到遮挡时游戏结束，其余列超出20行游戏结束，且在一次放置中同时有3列第20行被遮挡时游戏结束。
...
在Lv.11下，默认DAS无法在落地之前移动至场地边缘（Killscreen），但由于锁定延迟的存在依然可以进行极少量的消除（部分玩家除外）

# 技能消行
单局共可以进行20次技能释放：
- 1技能：立即将持有方块换为一个I块，无加成
- 2技能：在左侧第一列立刻填满20行方块并触发可能的消除，每行基础分值为$700+140(x-1)$，其中$x$为**消行后所处的等级**
> 2技能不计算高度加成，计算幸运加成，即持有幸运方块进行消除按消除高度计算幸运加成
> 
> 2技能消除完成后如果等级提升，下落速度依然为消除前的等级对应的下落速度
>
> 该基础分值表达式与“舞蹈”能力等级相关，测试中
- 3技能：立即将持有方块更换为同类型幸运方块
- 终结技：立即清空场地，场地上每一个方格基础分值10分，该技能消除的行数不计入等级
> 终结技计算高度加成，不计算幸运加成
>
> 终结技不消耗技能释放次数，每局只能使用一次
...
## 无助力，单局
### 能力加点：`[6,6,6,6]`
单次消除最高：4*(100+20*(i-1))*(7*4*5*(100+100*4)/100)=224000+56000i

假设全部进行4消，则在第2,4,7,10,14,19,24,29,34,39次4消处升级
